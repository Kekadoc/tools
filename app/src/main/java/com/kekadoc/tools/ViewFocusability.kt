...
/**
 * Когда какойто объект становится [ViewStateKey.FOCUS] все остальные объеты автоматически принимают
 * состояние [ViewStateKey.HIDED]
 * Когда последний элимент теряет состояние [ViewStateKey.FOCUS] то все остальные объеты должны
 * принять состояние [ViewStateKey.NORMAL]
 */
open class ViewFocusability<V : View> : DataStatesCollector<V, ViewFocusability.State, ViewFocusability<V>.StateView>() {

    companion object {
        private const val TAG: String = "ViewFocusability-TAG"
    }
