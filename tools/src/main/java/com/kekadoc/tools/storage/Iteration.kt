...
import com.kekadoc.tools.storage.collections.list.ListUtils
import java.util.*
...
object Iteration {
    ...
    fun <First, Second> twinsIteration(first: List<First?>, second: List<Second?>, iterator: (element_0: First?, element_1: Second?) -> Unit) {
        ...
        for (i in 0 until count) {
            val itemFirst: First? = ListUtils.getItemOrNull(first, i)
            val itemSecond: Second? = ListUtils.getItemOrNull(second, i)
