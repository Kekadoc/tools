package com.kekadoc.tools.storage.collections

import com.kekadoc.tools.ObjectUtils
import com.kekadoc.tools.text.StringUtils
import java.util.*
...
object CollectionUtils {
    ...
    fun <T> collectionToString(c: Collection<T>?): String {
        ...
        val builder = StringBuilder()
                .append(ObjectUtils.toSimpleString(c)).append(" size: ").append(c.size).append('\n')
        for (obj in c) {
    ...
    fun <T> toSimpleString(collection: Collection<T>?): String {
        if (collection == null) return StringUtils.NULL
        val builder = StringBuilder(ObjectUtils.toSimpleString(collection))
        builder.append("=").append("(").append(collection.size).append(")")
        builder.append("={")
        val iterator = collection.iterator()
        while (iterator.hasNext()) {
            val t = iterator.next()
            builder.append(ObjectUtils.toSimpleString(t))
