package com.kekadoc.tools.random

import com.kekadoc.tools.storage.array.ArrayUtils
...
object RandomUtils {
    ...
    @JvmStatic fun getRandomIndex(chance: IntArray): Int {
        val s: Int = ArrayUtils.getSum(chance)
        ...
        throw com.kekadoc.tools.exeption.Wtf()
    }
    @JvmStatic fun getRandomIndex(chance: FloatArray): Int {
        val s: Float = ArrayUtils.getSum(chance)
    ...
    @JvmStatic fun getRandomArray(start: Int, end: Int, count: Int, different: Boolean): IntArray {
        if (different && ValueUtils.getRangeValue(start, end) < count) throw com.kekadoc.tools.exeption.Wtf()
        ...
        for (i in 0 until count) {
            ...
            while (true) {
                ...
                if (different) {
                    if (!ArrayUtils.contain(rnd, r)) break
                } else break
            }
            rnd[i] = r
        }
    ...
    @JvmStatic fun getRandoms(from: IntArray, count: Int, differents: Boolean): IntArray {
        if (count > from.size) throw com.kekadoc.tools.exeption.Wtf()
